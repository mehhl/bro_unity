...
def evaluate_if_time_to(i, agent, eval_env, eval_interval, eval_episodes,
                        eval_returns, seeds, save_dir):
    if i % eval_interval == 0:
        eval_stats = eval_env.evaluate(agent, num_episodes=eval_episodes,
                                       temperature=0.0)
        for j, seed in enumerate(seeds):
            eval_returns[j].append((i, eval_stats['return'][j]))
            np.savetxt(os.path.join(save_dir, f'{seed}.txt'), eval_returns[j],
                       fmt=['%d', '%.1f'])
        log_to_wandb_if_time_to(i, eval_stats, eval_interval, suffix='_eval')


def make_env(benchmark, env_name, seed, num_envs, **unity_kwargs):
    if benchmark == 'dmc':
